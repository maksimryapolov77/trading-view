...
export class UDFCompatibleDatafeedBase implements IExternalDatafeed, IDatafeedQuotesApi, IDatafeedChartApi {
	...
	public calculateHistoryDepth(_resolution: ResolutionString, _resolutionBack: ResolutionBackValues, _intervalBack: number): HistoryDepth | undefined {
		return undefined;
	}
			...
			this._send<UdfSearchSymbolsResponse | UdfErrorResponse>('search', params)
				.then((response: UdfSearchSymbolsResponse | UdfErrorResponse) => {
					if (response.s !== undefined) {
						logMessage(`UdfCompatibleDatafeed: search symbols error=${(<UdfErrorResponse>response).errmsg}`); // modified by mars
						onResult([]);
						return;
					}

					onResult(<UdfSearchSymbolsResponse>response);  // modified by mars
				})
			...
			this._send<ResolveSymbolResponse | UdfErrorResponse>('symbols', params)
				.then((response: ResolveSymbolResponse | UdfErrorResponse) => {
					if (response.s !== undefined) {
						...
						onResultReady(<LibrarySymbolInfo>response); // modified by mars
					}
				})
				.catch((reason?: string | Error) => {
